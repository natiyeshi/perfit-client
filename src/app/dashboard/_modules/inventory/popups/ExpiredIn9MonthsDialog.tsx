...
const EXPIRED_9M_SEQ_ID = "16d7ccf0-cac7-4a82-9d3d-c9292a136c4d";
...
async function fetchExpired9MDetail(): Promise<{ rows: Array<Record<string, any>>; columns: string[] }> {
  const res = await axios.post<DetailResponse>(DETAIL_URL, {
    DashboardId: DASHBOARD_ID,
    DashboardItemSeqId: EXPIRED_9M_SEQ_ID,
  });
...
export default function ExpiredIn9MonthsDialog({ open, onOpenChange }: { open: boolean; onOpenChange: (v: boolean) => void }) {
  const { data, isLoading, isError, error, refetch } = useQuery(
    ["inventory-expired-9m-detail"],
    fetchExpired9MDetail,
    {
      enabled: open,
    }
  );
  ...
  return (
    <AlertDialog open={open} onOpenChange={onOpenChange}>
      <AlertDialogContent className="max-w-3xl">
        ...
        <AlertDialogHeader>
          <AlertDialogTitle className="text-base">Expired in 9 Months</AlertDialogTitle>
          <AlertDialogDescription className="text-xs">
            Products expiring within 9 months
          </AlertDialogDescription>
        </AlertDialogHeader>
