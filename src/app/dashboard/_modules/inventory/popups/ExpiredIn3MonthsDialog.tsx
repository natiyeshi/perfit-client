...
async function fetchExpired3MDetail(): Promise<{ rows: Array<Record<string, any>>; columns: string[] }> {
  const res = await axios.post<DetailResponse>(DETAIL_URL, {
    DashboardId: DASHBOARD_ID,
    DashboardItemSeqId: EXPIRED_3M_SEQ_ID,
  });
...
export default function ExpiredIn3MonthsDialog({ open, onOpenChange }: { open: boolean; onOpenChange: (v: boolean) => void }) {
  const { data, isLoading, isError, error, refetch } = useQuery(
    ["inventory-expired-3m-detail"],
    fetchExpired3MDetail,
    {
      enabled: open,
    }
  );
  ...
  return (
    <AlertDialog open={open} onOpenChange={onOpenChange}>
      <AlertDialogContent className="max-w-3xl">
        ...
        <AlertDialogHeader>
          <AlertDialogTitle className="text-base">Expired in 3 Months</AlertDialogTitle>
          <AlertDialogDescription className="text-xs">
            Products expiring within 3 months
          </AlertDialogDescription>
        </AlertDialogHeader>
