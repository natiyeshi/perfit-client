...
const AverageUnitPrice = ({
  ...
  const [tableData, setTableData] = useState<{ name: string; avgUnitPrice: number }[]>([]);
  ...
  useEffect(() => {
    ...
    importsData.forEach((d) => {
      ...
      if (filter.time === "all" || isAfter(createdAt, monthAgo)) {
        ...
        productMap[name].total += d.unitPrice;
  ...
  return (
    <div className="w-full  mx-auto mt-6 col-span-2 mb-12">
      ...
      <h3 className="text-xl text-center mb-2">Average Unit Price Per Product</h3>
      <table className="min-w-full border border-gray-200 rounded">
        ...
        <tbody>
          {tableData.filter(row => row.name.toLowerCase().includes(search.toLowerCase())).length === 0 ? (
            <tr>
            ...
            tableData
              ...
              .map((row) => (
                <tr key={row.name}>
                  ...
                  <td className="py-2 px-4 border-b">{row.avgUnitPrice.toFixed(2)}</td>
                </tr>
              ))
          )}
        </tbody>
